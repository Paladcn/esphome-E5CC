...
#define SDM_B_02 0x03 //BYTE 2 -> function code (default value 0x04 read from 3X input registers) 
                      //BYTES 3 & 4 (BELOW)
//Omron E5CC registers
#define E5CC_PV 0x0000 //℃
#define E5CC_SV 0x0602 //℃
...
#define SDM_B_06 0x02 //BYTE 6         这两个定义读取的字节长度
//------------------------------------------------------------------------------
...
{ //node就是modbus从站的站号，reg是读取地址，是16位的int，就是0000，分为高位和低位，
  uint16_t temp;
  unsigned long resptime;
  uint8_t sdmarr[FRAMESIZE] = {node, SDM_B_02, 0, 0, SDM_B_05, SDM_B_06, 0, 0, 0}; //SDM_B_05,06是读取的字节长度
  float res = NAN;
  uint16_t readErr = SDM_ERR_NO_ERROR;

  sdmarr[2] = highByte(reg);
  sdmarr[3] = lowByte(reg); //这里读取地址的高低位，赋予sdmarr命令数组

  temp = calculateCRC(sdmarr, FRAMESIZE - 3); //calculate out crc only from first 6 bytes

  sdmarr[6] = lowByte(temp); //添加计算的CRC的值。
  ...
  sdmSer.write(sdmarr, FRAMESIZE - 1); //send 8 bytes    发送485取值命令，为什么-1呢，因为都是使用sdmSer来发送和接收，发送是8位，回来是9位。
  ...
  { //if no timeout...
    ...
    if (sdmSer.available() >= FRAMESIZE)
    {
      ...
      if (sdmarr[0] == node && sdmarr[1] == SDM_B_02 && sdmarr[2] == SDM_REPLY_BYTE_COUNT)
      {
        ...
        if ((calculateCRC(sdmarr, FRAMESIZE - 2)) == ((sdmarr[8] << 8) | sdmarr[7]))
        { //calculate crc from first 7 bytes and compare with received crc (bytes 7 & 8)
          // ((uint8_t*)&res)[3]= sdmarr[3];                                      //sdmarr已经是读回来的值了，关键转成res的浮点小数怎么转换
          //((uint8_t*)&res)[2]= sdmarr[4];                                      // 将变量res的地址&res强制转换为uint8_t * 的指针类型
          //((uint8_t*)&res)[1]= sdmarr[5];
          //((uint8_t*)&res)[0]= sdmarr[6];
          //uint8_t temparr[2]=(sdmarr[5],sdmarr[6]);

          int Dec1, Dec2;
          char result[16], buf[8];
          char *stop;
          Dec1 = sdmarr[5];
          Dec2 = sdmarr[6];
          itoa(Dec1, result, 16);
          itoa(Dec2, buf, 16);
          strcat(result, buf);      //结合两个转换好的16进制数
          res=strtol(result,&stop,16);
          res=res/10;
        }
        else
        {
